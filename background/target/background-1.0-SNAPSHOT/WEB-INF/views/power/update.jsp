...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    <div class="container-fluid">
        <div class="navbar-header">
            <div><a class="navbar-brand" style="font-size:32px;" href="user.html">许可维护 许可更新</a></div>
        </div>
                ...
                <div class="panel-body">
                    <form role="form">
                        <div class="form-group">
                            <label >新的权限名</label>
                            <input type="text" class="form-control" id="powername" value="${power.name}" ><span id="mySpan"></span>
                        </div>
                        <div class="form-group">
                            <label >新的url</label>
                            <input type="text" class="form-control" id="powerurl"  value="${power.url}"><span id="mySpan1"></span>
                        </div>

                        <button type="button" id="insert" class="btn btn-success"><i class="glyphicon glyphicon-plus"></i> 修改</button>
                        <button id="flush" type="button" class="btn btn-danger"><i class="glyphicon glyphicon-refresh"></i> 重置</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    $("#powername").blur(function () {
        ...
        $.ajax({
                ...
                success:function (result) {
                    if(!result.result&&powername!=name)
                        $("#mySpan").html("权限名已经存在");
                    else
                        $("#mySpan").html("");
                }
    ...
    $("#flush").click(function () {
        window.location.href="${PATH}/power/update?id=${power.id}";
    })
