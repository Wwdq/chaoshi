...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    <div class="container-fluid">
        <div class="navbar-header">
            <div><a class="navbar-brand" style="font-size:32px;" href="user.html">个人信息--修改个人信息</a></div>
        </div>
                ...
                <div class="panel-body">
                    <form role="form">
                        <div class="form-group">
                            <label >密码</label>
                            <input type="password" class="form-control" id="username" ><span id="mySpan"></span>
                        </div>
                        <div class="form-group">
                            <label >确认密码</label>
                            <input type="password" class="form-control" id="username1"  ><span id="mySpan1"></span>
                        </div>
                        <div class="form-group">
                            <label  >手机号</label>
...
<script>
    ...
    $("#username1").blur(function () {
        ...
        if(username==username1)
            ...
            $("#mySpan1").html("两次输入的密码不一样 请检查");
    })
    $("#flush").click(function () {
        window.location.href="${PATH}/user/password?id=${user.id}";
    })
    $("#update").click(function () {
        ...
        $.ajax({
            url:"${PATH}/user/passwordUpdate?id=${user.id}",
            type: "POST",
            data: {
                "password":username,
                "telephone" :phone
            ...
            success : function (result) {
                if(result.result){
                    layer.msg("修改密码成功 请重新登录", {time:2000, icon:4, shift:6}, function () {
                        window.location.href="${PATH}/doOut";
                    });
