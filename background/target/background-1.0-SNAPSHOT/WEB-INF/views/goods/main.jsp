...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    <div class="container-fluid">
        <div class="navbar-header">
            <div><a class="navbar-brand" style="font-size:32px;" href="#"> 商品一览</a></div>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <button onclick="goSelect()" class="btn btn-default" type="button">查找</button>
                                 </span>
                        </div><!-- /input-group -->
                    </div><!-- /.col-lg-6 -->

                    <button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${PATH}/goods/add'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
                    <br>
                    <hr style="clear:both;">
                    <div class="table-responsive">
                       ...
                       <c:forEach items="${list}" var="now" varStatus="count">
                                <tr>
                                    ...
                                    <td>${now.price}</td>
                                    <td>${now.disPrice}</td>
                                    <td><c:if test="${now.discount==0}">未促销</c:if>
                                        <c:if test="${now.discount==1}">促销</c:if></td>
                                    <td>
                                        <button type="button" onclick="toMai(${now.id})" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>
                                        <button type="button"  onclick="update(${now.id})" class="btn btn-primary btn-xs"><i    class=" glyphicon glyphicon-pencil"></i></button>
                                        <button type="button" onclick="deleteGoods(${now.id})" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>
                                    </td>
                                </tr>
                       </c:forEach>
                            </tbody>
...
<script >
    ...
    function toMai(id){
        layer.confirm("确认促销此商品么",  {icon: 3, title:'成功'}, function(cindex){
            window.location.href="${PATH}/goods/discount?id="+id;
            layer.close(cindex);
        }, function(cindex){
            layer.close(cindex);
        });
    ...
    function deleteGoods(id){
        layer.confirm("确认删除么",  {icon: 3, title:'删除成功'}, function(cindex){
            window.location.href="${PATH}/goods/delete?id="+id;
            layer.close(cindex);
        }, function(cindex){
            layer.close(cindex);
        });
